        ...
        if (e.type == sf::Event::KeyPressed)
        {
            if (m_State == GameState::MENU)
            {
                ...
                else if (e.key.code == sf::Keyboard::Enter || e.key.code == sf::Keyboard::Space)
                {
                    ...
                    else if (m_MenuIndex == 1) { m_ShowSettings = true; m_SettingsMainMenu.show(); }
                    else if (m_MenuIndex == 2) m_Window.close();
                }
                else if (e.key.code == sf::Keyboard::Escape) m_Window.close();
            }
            else if (m_State == GameState::PLAYING && e.key.code == sf::Keyboard::Escape)
            {
            ...
            else if (m_State == GameState::PAUSED)
            {
                ...
                else if (e.key.code == sf::Keyboard::Escape) m_State = GameState::PLAYING;
                else if (e.key.code == sf::Keyboard::Enter || e.key.code == sf::Keyboard::Space)
                {
            ...
            {
                if (e.key.code == sf::Keyboard::Enter || e.key.code == sf::Keyboard::Space) newGame();
                else if (e.key.code == sf::Keyboard::Escape) { m_State = GameState::MENU; m_MenuIndex = 0; refreshMenuHighlight(); }
            }
        }
        if (e.type == sf::Event::MouseButtonPressed && e.mouseButton.button == sf::Mouse::Left && m_State == GameState::MENU && !m_ShowSettings)
        {
            ...
            for (size_t i = 0; i < m_MenuTexts.size(); ++i)
            {
                if (m_MenuTexts[i].getGlobalBounds().contains(mp))
                {
                    ...
                    refreshMenuHighlight();
                    if (i == 0) newGame();
                    else if (i == 1) { m_ShowSettings = true; m_SettingsMainMenu.show(); }
...
void Game::updatePlaying(float dt)
{
    ...
    if (Input::isMouseButtonPressed(sf::Mouse::Left)) handlePlayerShoot();
    ...
    for (int iter = 0; iter < COLLISION_ITERATIONS; ++iter)
    {
        for (size_t i = 0; i < m_Enemies.size(); ++i)
        {
            ...
            pos.x = std::max(margin, std::min(pos.x, (float)(Room::GRID_WIDTH * Room::TILE_SIZE) - margin));
            pos.y = std::max(margin, std::min(pos.y, (float)(Room::GRID_HEIGHT * Room::TILE_SIZE) - margin));
            e->setPosition(pos);
    ...
    // Снаряды
    for (auto& p : m_Projectiles) p.update(dtClamped);
    for (auto& proj : m_Projectiles)
    {
        ...
        if (proj.isFromPlayer())
        {
            for (auto& e : m_Enemies)
            {
                ...
                {
                    e->takeDamage((int)proj.getDamage());
                    e->flashHit();
                    if (!proj.canPierce()) proj.kill();
                    if (!e->isAlive())
                    {
    ...
    // Враждебные снаряды против игрока
    sf::FloatRect playerBounds = m_Player.getBounds();
    for (auto& proj : m_Projectiles)
    {
        if (proj.isDead()) continue;
        if (!proj.isFromPlayer() && proj.getBounds().intersects(playerBounds))
        {
            m_Player.damage((int)proj.getDamage());
            proj.kill();
    ...
    {
        for (auto& e : m_Enemies)
            if (e->isAlive() && e->canContactDamage() && e->getBounds().intersects(m_Player.getBounds()))
            {
                m_Player.damage((int)e->getDamage());
                if (!m_Player.isAlive()) break;
            }
    }
    ...
    for (auto& wi : m_WorldItems)
    {
        if (wi.collected || wi.roomIndex != currentRoomIdx) continue;
        if (wi.item.getBounds().intersects(m_Player.getBounds()))
        {
            if (m_Level.getCurrentNode().type == RoomType::SHOP && wi.item.getCost() > 0)
            {
                if (Input::isKeyJustPressed(sf::Keyboard::E))
                {
                    ...
                    else m_HUD.setMessage("Not enough coins (" + std::to_string(wi.item.getCost()) + ")", 1.5f);
                }
                else m_HUD.setMessage("Press E to buy for " + std::to_string(wi.item.getCost()) + " coins", 0.1f);
            }
            else
            {
                m_Player.applyItem(wi.item.getType());
...
void Game::checkRoomTransitions()
{
    ...
    for (int d = 0; d < 4; ++d)
    {
        ...
        if (m_Room.isAtDoor(m_Player.getBounds(), dir))
        {
            Direction fromDir;
...
void Game::loadCurrentRoom()
{
    ...
    unsigned int roomSeed = (unsigned)(currentRoomIdx * 131 + m_Level.getFloorNumber() * 977 +
        m_Level.getCurrentNode().x * 31 + m_Level.getCurrentNode().y * 17 + 1);
    m_Room.generate(type, doors, roomSeed);

    for (auto it = m_WorldItems.begin(); it != m_WorldItems.end(); )
    {
        if (it->roomIndex == currentRoomIdx && it->collected) it = m_WorldItems.erase(it);
        else ++it;
    }
    ...
    if (!m_Level.getCurrentNode().cleared)
    {
        bool hasItems = false;
        for (auto& wi : m_WorldItems)
            if (wi.roomIndex == currentRoomIdx && !wi.collected) { hasItems = true; break; }
        if (!hasItems) spawnRoomContent();
    }
    ...
    else if (type == RoomType::SHOP) m_HUD.setMessage("Shop: Approach item and press E", 2.5f);
    else if (type == RoomType::ITEM) m_HUD.setMessage("Item Room", 1.5f);
}

void Game::spawnRoomContent()
{
    std::mt19937 rng((unsigned)(m_Level.getCurrentRoomIndex() * 7919 + m_Level.getFloorNumber() * 101 +
        m_Level.getCurrentNode().x * 17 + m_Level.getCurrentNode().y * 31));
    RoomType type = m_Level.getCurrentNode().type;
    ...
    if (type == RoomType::NORMAL && !m_Level.getCurrentNode().cleared)
    {
        ...
        auto points = m_Room.getSpawnPoints(enemyCount, (int)(m_Level.getCurrentRoomIndex() + floor * 31), defaultRadius);
        for (auto& p : points)
        {
            int roll = (int)(rng() % 100);
            EnemyType et = (roll < 55) ? EnemyType::SLIME : (roll < 85) ? EnemyType::SHOOTER : EnemyType::BERSERKER;
    ...
    else if (type == RoomType::ITEM && !m_Level.getCurrentNode().cleared)
    {
        ...
        int roll = (int)(rng() % 4);
        ItemType it = (roll == 0) ? ItemType::HEALTH : (roll == 1) ? ItemType::DAMAGE_UP : (roll == 2) ? ItemType::SPEED_UP : ItemType::SHIELD;
...
void Game::render()
{
    ...
    if (m_State == GameState::MENU && !m_ShowSettings) drawMenu();
    else if (!m_ShowSettings)
    {
        drawPlaying();
        if (m_State == GameState::PAUSED) drawOverlay("PAUSED", "Enter - resume, Esc - menu", sf::Color(230, 230, 230));
        else if (m_State == GameState::GAME_OVER) drawOverlay("GAME OVER", "Score: " + std::to_string(m_Score) + "   Best: " + std::to_string(m_HighScore) + "\nEnter - new game, Esc - menu", sf::Color(230, 80, 80));
        else if (m_State == GameState::VICTORY) drawOverlay("VICTORY!", "Score: " + std::to_string(m_Score) + "   Best: " + std::to_string(m_HighScore) + "\nEnter - new game, Esc - menu", sf::Color(120, 240, 140));
    }
    if (m_ShowSettings) { m_SettingsMainMenu.draw(m_Window); SettingsManager::getInstance().draw(m_Window); }
    m_Window.display();
}
...
void Game::drawPlaying()
{
    ...
    int currentRoomIdx = m_Level.getCurrentRoomIndex();
    for (auto& wi : m_WorldItems)
        if (wi.roomIndex == currentRoomIdx && !wi.collected) wi.item.draw(m_Window);
    ...
    m_HUD.draw(m_Window);
    if (m_UpgradeUI.isActive()) m_UpgradeUI.draw(m_Window);
}
...
void Game::drawOverlay(const std::string& title, const std::string& subtitle, sf::Color titleColor)
{
    ...
    sf::Text t; t.setFont(font); t.setString(title); t.setCharacterSize(72); t.setFillColor(titleColor);
    t.setOutlineColor(sf::Color::Black); t.setOutlineThickness(3.f);
    auto tb = t.getLocalBounds(); t.setOrigin(tb.left + tb.width / 2.f, tb.top + tb.height / 2.f);
    t.setPosition(WIN_WIDTH / 2.f, 220.f); m_Window.draw(t);

    sf::Text s; s.setFont(font); s.setString(subtitle); s.setCharacterSize(22); s.setFillColor(sf::Color(230, 230, 230));
    auto sb = s.getLocalBounds(); s.setOrigin(sb.left + sb.width / 2.f, sb.top + sb.height / 2.f);
    s.setPosition(WIN_WIDTH / 2.f, 330.f); m_Window.draw(s);
    ...
    if (m_State == GameState::PAUSED)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            sf::Text mi; mi.setFont(font); mi.setString(items[i]); mi.setCharacterSize(28);
            mi.setFillColor(i == m_MenuIndex ? sf::Color(255, 220, 100) : sf::Color(200, 200, 200));
            auto lb = mi.getLocalBounds(); mi.setOrigin(lb.left + lb.width / 2.f, lb.top + lb.height / 2.f);
            mi.setPosition(WIN_WIDTH / 2.f, 420.f + i * 42.f); m_Window.draw(mi);
        }
    }
}
...
void Game::loadHighScore()
{
    ...
    if (f.is_open()) { f >> m_HighScore; if (!f) m_HighScore = 0; }
    else m_HighScore = 0;
}
...
void Game::checkAndShowUpgradeUI()
{
    ...
    while (m_Player.getExpSystem().hasPendingUpgrade() && !m_UpgradeUI.isActive())
    {
        auto choices = m_Player.getExpSystem().getUpgradeChoices(3);
        if (!choices.empty())
        {
        ...
        {
            m_Player.getExpSystem().clearAllPendingUpgrades();
            break;
        }
    }
}